...
import com.ping.homeschool.config.exception.CommonJsonException;
import com.ping.homeschool.util.constants.Constants;
...
public class CommonUtil {
	...
	/**
	 * 将requestForm的字符串转为map
	 */
	public static Map<String, String> StringTurnToMap(String str){
		if(!StringTools.isNullOrEmpty(str)){
			...
			for (String column : columns) {
				...
				String key = StringTools.trim(keyValue[0].trim(), "\"");
				// 若表单项目为空，情况则是：  ""，共3个字符，故要先大于3
				String value = "";
				if(keyValue[1].length() > 0) value = StringTools.trim(keyValue[1].trim(), "\"");
				map.put(key, value);
			}
			return map;
		}
